...
Z0   = sqrt(mu0/eps0);     % Free space wave impedance (Ohms)
eta0 = Z0;                 % Alternative notation for free space impedance

% Commonly Used Derived Constants
f0      = 1e9;             % Example: Center frequency (Hz), adjust as needed
lambda0 = c0 / f0;         % Wavelength in free space (m)
omega0  = 2 * pi * f0;     % Angular frequency (rad/s)
k0      = omega0 / c0;     % Free-space wave number (1/m)

% Conductivity & Dielectric Properties (Example: FR4)
epsR    = 4.3;             % Relative permittivity (dielectric constant)
tand    = 0.02;            % Loss tangent
sigma   = omega0 * eps0 * epsR * tand; % Conductivity (S/m)

% Mesh and Discretization (For FDTD)
dx      = lambda0 / 20;    % Cell size (rule of thumb: λ/10 to λ/20)
dt      = dx / (2 * c0);   % Time step (CFL condition for stability)

% Magnetic & Electric Properties
gamma   = sqrt(1i * omega0 * mu0 * sigma - (omega0^2) * eps0 * mu0); % Propagation constant
